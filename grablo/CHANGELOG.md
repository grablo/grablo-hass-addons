...
- Cards now appear as the camera sees people, instead of several seconds later — the indexer no longer reopens the video to build them.
- Cameras running at low frame rates (around 3 fps) now keep a stable track. The tracker used to hand out a new number every couple of seconds, which broke grouping.
- People who walk behind each other or cross paths are followed through the occlusion rather than swapped.
- Searching by person, face or words is substantially faster, and a 30-day search no longer slows down as the database grows.
...
- Fixed a crash that could take the controller down after the first STOP, and removed diagnostic noise from log.txt.
